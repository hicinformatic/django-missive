    ...
    def get_queryset(self):
        ...
        for provider_name, missive_types in sorted(providers_dict.items()):
            provider = ProviderInfo(
                pk=pk,
                name=provider_name,
                missive_type=','.join(missive_types),  # Stocker tous les types séparés par virgule
            )
...
class ProviderInfo(models.Model):
    ...
    def _get_provider_class(self):
        ...
        try:
            ...
            # Normaliser le nom du module (minuscules, sans espaces ni tirets)
            module_name = self.name.lower().replace(' ', '').replace('-', '')
            
            # Cas spéciaux pour le nom de la classe
            class_name_map = {
                'djangoemail': 'DjangoEmailProvider',
                'django_email': 'DjangoEmailProvider',
            ...
            if not class_name:
                ...
                class_name = f'{module_name.capitalize()}Provider'
            
            # Construire le chemin du provider
            provider_path = f'missive.providers.{module_name}.{class_name}'
    ...
    def required_package(self):
        """
        Retourne le nom du package Python requis pour ce provider.
        Récupère le required_package directement depuis la classe du provider.
        """
        provider_class = self._get_provider_class()
        if provider_class is None:
            return None
        return getattr(provider_class, 'required_package', None)
    ...
    def requirements_file(self):
        ...
        # Mapping provider → fichier requirements
        mapping = {
            # Email
            'sendgrid': 'requirements-email.txt',
            'mailgun': 'requirements-email.txt',
            'ses': 'requirements-email.txt',
            # SMS
            'twilio': 'requirements-sms.txt',
            'vonage': 'requirements-sms.txt',
            'smspartner': 'requirements-sms.txt',
            # Messaging
            'telegram': 'requirements-messaging.txt',
            'signal': 'requirements-messaging.txt',
            'messenger': 'requirements-messaging.txt',
            # Push
            'fcm': 'requirements-push.txt',
            'apn': 'requirements-push.txt',
            # Pro
            'slack': 'requirements-professional.txt',
            'teams': 'requirements-professional.txt',
            # Postal
            'laposte': 'requirements-postal.txt',
            'ar24': 'requirements-postal.txt',
            'certeurope': 'requirements-postal.txt',
        }
    ...
    def credits_info(self):
        ...
        try:
            ...
            # Construire le chemin du provider
            provider_path = f'missive.providers.{self.name}.{self.name.capitalize()}Provider'
