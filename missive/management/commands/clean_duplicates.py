...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        emails = Recipient.objects.exclude(email='').exclude(email__isnull=True).values('email').annotate(count=Count('id')).filter(count__gt=1)
        ...
        for item in emails:
            ...
            for dup in to_delete:
                self.stdout.write(self.style.WARNING(f'    ❌ Suppression: {dup.name} (ID: {dup.id})'))
                dup.delete()
                removed_count += 1
        
        # 2. Nettoyer les doublons de mobile
        self.stdout.write('\n🔍 Recherche de doublons de mobile...')
        mobiles = Recipient.objects.exclude(mobile='').exclude(mobile__isnull=True).values('mobile').annotate(count=Count('id')).filter(count__gt=1)
        ...
        addresses = Recipient.objects.exclude(address_line1='').exclude(address_line1__isnull=True).values(
            'name', 'address_line1', 'postal_code', 'city'
        ...
        for item in addresses:
            ...
            self.stdout.write(f'  📍 {item["name"]} - {item["address_line1"]}: {duplicates.count()} doublons')
            ...
            for dup in to_delete:
                self.stdout.write(self.style.WARNING(f'    ❌ Suppression: ID {dup.id}'))
                dup.delete()
                removed_count += 1
        ...
        self.stdout.write(self.style.SUCCESS(f'✓ Total: {Recipient.objects.count()} destinataires'))
