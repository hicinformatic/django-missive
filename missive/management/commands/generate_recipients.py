    ...
    def handle(self, *args, **options):
        ...
        for recipient_data in recipients:
            ...
            if created:
                ...
                self.stdout.write(
                    self.style.SUCCESS(f'  {icon} {recipient.full_name} - {recipient.primary_contact or "Pas de contact"}')
                )
        ...
        # Statistiques
        self.stdout.write('\n📊 Statistiques:')
        self.stdout.write(f'  - Expéditeurs: {Recipient.objects.filter(can_be_sender=True).count()}')
        self.stdout.write(f'  - Avec email: {Recipient.objects.exclude(email="").exclude(email__isnull=True).count()}')
        self.stdout.write(f'  - Avec mobile: {Recipient.objects.exclude(mobile="").exclude(mobile__isnull=True).count()}')
        self.stdout.write(f'  - Avec adresse: {Recipient.objects.exclude(address_line1="").exclude(address_line1__isnull=True).count()}')
        self.stdout.write(f'  - Actifs: {Recipient.objects.filter(is_active=True).count()}')
