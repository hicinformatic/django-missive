...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not sender:
            self.stdout.write(self.style.ERROR('❌ Aucun expéditeur disponible. Lancez "python dev.py generate_recipients" d\'abord.'))
            return

        # Récupérer des destinataires selon leur type de contact
        email_recipients = Recipient.objects.exclude(email='').exclude(email__isnull=True).filter(can_be_sender=False)[:3]
        mobile_recipients = Recipient.objects.exclude(mobile='').exclude(mobile__isnull=True).filter(can_be_sender=False)[:3]
        postal_recipients = Recipient.objects.exclude(address_line1='').exclude(address_line1__isnull=True).filter(can_be_sender=False)[:3]
        ...
        for i, recipient in enumerate(mobile_recipients[:2], 1):
            missives.append({
                'missive_type': 'WHATSAPP',
                'sender': sender,
                'recipient': recipient,
                'subject': f'WhatsApp {i}',
                'body': f'Bonjour {recipient.full_name}, message WhatsApp {i}',
                'priority': 'NORMAL',
        ...
        self.stdout.write(self.style.SUCCESS(f'✓ Total: {Missive.objects.count()} missives'))
        
        # Statistiques
        self.stdout.write('\n📊 Statistiques par type:')
        for missive_type, label in [('EMAIL', 'Email'), ('SMS', 'SMS'), ('WHATSAPP', 'WhatsApp'), ('POSTAL', 'Postal'), ('NOTIFICATION', 'Notification')]:
            count = Missive.objects.filter(missive_type=missive_type).count()
            self.stdout.write(f'  - {label}: {count}')
        
        self.stdout.write('\n📊 Statistiques par statut:')
        for status, label in [('DRAFT', 'Brouillon'), ('PENDING', 'En attente'), ('SENT', 'Envoyé')]:
